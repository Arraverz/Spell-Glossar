<template lang="html">
  <li
    ...
    <button
      ...
      <feather-icon v-if="icontype == 'default'" :icon="value"></feather-icon>
      <font-awesome-icon v-else-if="icontype == 'fontawesome'" :icon="value"/>
      <span v-else-if="icontype == 'metro'" :class="'mif-2x mif-' + value"></span>
...
export default {
  ...
  computed: {
    ...
    styles() {
      return {
        background: this.isActive ? '#fff' : null,
        color: this.isActive ? '#000' : null,
